...
namespace dxvk {
  ...
  DxsoRegisterValue DxsoCompiler::emitMul(
          DxsoRegisterValue       a,
          DxsoRegisterValue       b) {
    a = emitMulOperand(a, b);
    b = emitMulOperand(b, a);

    DxsoRegisterValue result;
    result.type = a.type;
    result.id = m_module.opFMul(getVectorTypeId(result.type), a.id, b.id);
    return result;
  }
  ...
  DxsoRegisterValue DxsoCompiler::emitFma(
    ...
    DxsoRegisterValue result;
    result.type = a.type;
    result.id = m_module.opFFma(getVectorTypeId(result.type), a.id, b.id, c.id);
    return result;
  }
